...
use numpy::{ndarray::Array, ndarray::Array2, ndarray::Array3, ndarray::stack, ndarray::s, ndarray::Axis};
// use numpy::{PyReadonlyArray2, PyArray2};
// use pyo3::{prelude::*, Python};


// ---------- internal helper, NOT exposed to Python ----------
fn daubechies8() -> Array3<f32> {
    let hpdf: [f32; 16] = [
...
fn pad_symmetric(input: &Array2<f32>, pad: usize) -> Array2<f32> {
    let (h, w) = input.dim();
    let mut output = Array2::<f32>::zeros((h + 2*pad, w + 2*pad));

    for i in 0..output.nrows() {
        for j in 0..output.ncols() {
            let ii = reflect_index(i as isize - pad as isize, h as isize);
            let jj = reflect_index(j as isize - pad as isize, w as isize);
...
fn convolve2d(input: &Array2<f32>, kernel: &Array2<f32>) -> Array2<f32> {
    ...
    let input_pad = pad_symmetric(input, pad_h.max(pad_w));
...
fn compute_cost<'py>(py: Python<'py>, x0: PyReadonlyArray2<'py, u8>, p: f32)
    ...
    let (h, w) = input.dim();
    let mut x0_pad = pad_symmetric(&input, 16 as usize);
